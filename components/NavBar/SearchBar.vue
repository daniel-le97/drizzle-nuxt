    ...
    <div class="dropdown">
      ...
      <ul v-if="showResultsList" tabindex="0"
        ...
        <div class="max-h-96 overflow-y-scroll p-3">
          <li v-if="searchResults.length > 0" class="text-xs font-medium ">Results:</li>
          <li v-else class="text-xs font-medium text-error ">No Results</li>
          <div class="divider m-0"></div>
          <li v-motion-slide-right v-if="searchResults.length > 0" v-for="result in searchResults" :key="result.id">
            <NuxtLink :to="`/dynamic/${result.id}`" class="p-2 btn w-full btn-ghost  font-medium">
              <span v-html="highlightMatch(result.first_name)"></span>
            </NuxtLink>
          </li>
        </div>
      </ul>
